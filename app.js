...
function habitatMapInfo(mapKey) { const map = habitatData?.maps?.[mapKey] || {}; const image = String(map.image || '').startsWith('http') ? map.image : `https://pal-kit.com/${String(map.image || (mapKey === 't' ? 'habitat-map/tree.webp' : 'habitat-map/world.webp')).replace(/^\/+/, '')}`; const bounds = map.bounds || {}; const rawSize = bounds.size || map.size || 2048; const width = Number(typeof rawSize === 'object' ? (rawSize.width || rawSize.x || 2048) : rawSize) || 2048; const height = Number(typeof rawSize === 'object' ? (rawSize.height || rawSize.y || width) : rawSize) || width; return { ...map, image, width, height, bounds }; }
function decodeHabitatPoints(encoded, mapInfo) { if (!encoded) return []; if (Array.isArray(encoded)) return encoded.map(point => ({ x: Number(point.x ?? point[0]) || 0, y: Number(point.y ?? point[1]) || 0 })); try { const encodedText = String(encoded); const normalized = encodedText.replace(/-/g, '+').replace(/_/g, '/').padEnd(Math.ceil(encodedText.length / 4) * 4, '='); const binary = atob(normalized); const bytes = new Uint8Array(binary.length); for (let index = 0; index < binary.length; index += 1) bytes[index] = binary.charCodeAt(index); const points = []; for (let index = 0; index + 3 < bytes.length; index += 4) { const rawX = bytes[index] | (bytes[index + 1] << 8); const rawY = bytes[index + 2] | (bytes[index + 3] << 8); points.push({ x: (rawX / 65535) * mapInfo.width, y: (rawY / 65535) * mapInfo.height }); } return points; } catch (error) { return []; } }
