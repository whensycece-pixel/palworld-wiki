...
function applyLanguage(language, announce = false) {
  ...
  if (window.palworldAppReady) { renderPals(); renderEggs(); renderTowers(); renderAlphaList(); renderMapPins(); updateWorldListSummary(); updateMapRegionUi(); updateDatabaseCounts(); renderDatabase(); renderBreedDirectory(); updateBreedDirectorySlots(); updateCapture(); renderTiers(); updateBreedOutput(); if (!document.querySelector('#breedResultPanel')?.hidden) updateReverseBreedOutput(); }
